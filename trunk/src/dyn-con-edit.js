            ...
            <>
                <InspectorControls>
                    <div className="sidebar">
                        ...
                        {this.state.showDropdown ? <select name="dynamicContentSelect" value={this.props.attributes.dynamicContent_id}
                                                           onChange={e => this.handleDropdownChange(e)}>
                            <option value={-1} key={-1} label={"Select"}>Select</option>
                            {this.state.dropdownItems.map(function (n) {
                                return (<option label={n.title} value={n.dynamicContent_id} key={n.dynamicContent_id}>{n.title}</option>);
                            })}
